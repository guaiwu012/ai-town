...
- 角色应援与阵营经营：观众选择本命角色和猎手、后勤、情报三条阵营路线，用 1/3/5 干预点发布 60 秒悬赏追猎、物资搜集或接触谈判任务。AI 会按人设与战况接受、拒绝或提出加码；接受后规则 AI 与 DeepSeek 都会优先执行，直播 HUD 显示进度，成功返还干预点并积累本地阵营声望。
- 已接受的观众任务由服务端强制驱动：追猎者会停止普通巡逻，沿区域邻接图跨区接近目标，直接规划到当前武器射程并持续攻击，不受 DeepSeek 12 秒调度节奏阻塞。
- 首次任务引导：进入直播后展示三步观众任务说明，解释应援角色、阵营路线、60 秒指令与奖励，并可直接进入应援作战室；完成或略过后不重复打断观赛。
