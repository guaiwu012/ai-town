...
- 角色应援与阵营经营：观众选择本命角色和猎手、后勤、情报三条阵营路线；作战室会依据同区接触、资源缺口和低血量敌人生成实时战术机会，并预估接受率、行动风险与成功返还。观众每 40 秒可用 1/3/5 干预点发布一项 55 秒任务，角色拒绝会立即退款，接受后规则 AI 与 DeepSeek 都会优先执行。
- 三段战术连携：按“接触谈判 → 物资搜集 → 悬赏追猎”推进任务链，完成后可释放猎手、后勤或情报路线终结技。直播 HUD 会显示跨区追踪、接近谈判、抢占射击位和交火等分段进度，连续成功还会提高本地阵营声望收益。
- 已接受的观众任务由服务端强制驱动：追猎者会停止普通巡逻，沿区域邻接图跨区接近目标，直接规划到当前武器射程并持续攻击，不受 DeepSeek 12 秒调度节奏阻塞。
- 首次任务引导：进入直播后展示三步观众任务说明，解释应援角色、阵营路线、55 秒指令与奖励，并可直接进入应援作战室；完成或略过后不重复打断观赛。
- 可执行角色卡：12 名角色拥有独立目标、战斗方式、攻击/结盟/撤退权重和中文语气；规则 AI 与 DeepSeek 共用这些人设约束，同区相遇后按人设和关系选择开战、结盟或撤离。
- Convex 实时比赛状态：生命、武器、物资、区域、禁区、热度、连击、任务、线索和真相之间。
- 主办方干预点：扫雷小游戏结算干预点；可投放补给、陷阱、情报和规则干预。“悬赏追杀”以第一角色为任务执行者、第二角色为追杀目标，执行者会跨区追踪且只有亲自完成淘汰才能获得奖励。
- 24 条区域特殊剧情和 3 条全局事件已进入事件循环，地图会显示干预和剧情落点。
- 近距离遭遇默认把陌生或负关系角色视为敌对目标并提高攻击权重；已有信任关系仍可协商，低生命角色仍保留撤离策略。直播层会限制对话显示时长和气泡数量，非焦点地标不再持续显示名称。
- 直播观赛模式：默认自动导播跟随热点 AI；可切换战略总览、锁定角色镜头、查看中文决策审计，并从直播 HUD 直接发起扫雷或新开一局。
- 持续行动与交谈：角色使用独立于 12 秒战术决策的 A* 巡逻循环，一边移动一边等待模型决策；结盟和交易会显示中文地图气泡，并保存双方台词供角色详情查询。
- 全局战术感知：每个 AI 都知道所有存活角色的姓名、区域、精确坐标与距离；DeepSeek 获得跨区最短下一跳，规则 AI 也会利用同一全局位置选择和追踪目标。
- 强制近距反应：同区 AI 距离进入 2.2 格后，服务端即时反射层必须执行射击或一轮双人交谈；试探、停火、警告与同伴交流都会改变双方关系，并使用 18 秒配对冷却避免刷屏。
- 直播交锋表现：角色开火后持续锁定目标并追到射击位，受击者会反向锁定威胁；手枪双发、步枪四发短点射显示独立枪口闪光、曳光弹与命中冲击，人物同步表现后坐、受击抖动、红色闪身和生命变化。移动状态会区分 `MOVE`、追击目标、悬赏追击、撤离、搜索补给和规划路线。
- 场内角色美术：12 名参赛者拥有与档案头像对应的原创俯视角全身精灵；直播战场使用步态起伏、朝向翻转、脚底跟拍环、独立生命条和淘汰态，不再以缩小头像圆章代替角色。
- 跑团式区域剧情：24 条区域事件各自提供稳妥处置、抢先突破、协同应对三种事件专属选择，包含场景、D20 属性检定、成功/失败叙事和不同规则后果；DeepSeek 会明确选择事件 ID 与处置方案，服务端校验区域、次数和剧情道具后才执行。
- AI 剧情路线：DeepSeek 执行调查时必须在“谨慎勘察、强行突破、交涉取证”中选择；路线会改变检定属性、难度、热度或关系结果，并进入中文决策审计和回放行动日志。
- P2 回放：比赛保存种子、RNG 状态、结构化模型/规则行动、紧凑状态补丁和每 30 秒轻量检查点。回放从最近检查点按 `actionId` 应用补丁，不重复请求 DeepSeek；角色位置、生命、装备、区域、关系、资源、剧情、禁区和干预点都会恢复到所选时刻。
- P2 战场导航：13 区多边形、参考邻接走廊、出生点和 26 个地标障碍会生成一张确定性导航栅格。服务端 A*、重置出生和 Pixi 障碍反馈读取同一份关卡数据，角色不会再把旧 AI Town 墙体当作当前战场碰撞。
- 云端 DeepSeek 战术驾驶：观众客户端用 20 秒租约避免重复扣费；Convex Action 用云变量调用模型，动作经区域邻接、禁区、射程、物资和冷却校验后才会执行。
- 首次进入播放项目开场视频并叠加三幕中文世界观介绍；接入直播信号时先通过唯一的 `world.resetBattle` 权威入口创建新局，初始化完成后再以扫描和闪白转场进入战场。返回页面会继续观看当前公共比赛，不会因任意观众刷新而重置。游戏内使用 CC0 暗黑科幻循环音乐和事件音效，许可与来源见 [音频资源说明](./docs/audio-assets.md)。
