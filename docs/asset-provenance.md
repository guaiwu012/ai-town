...
参考仓库 commit 中的全部 108 个受版本控制文件（其中 50 个视觉文件）均已登记 SHA-256 与落地方式，完整清单见 [`reference-artifact-inventory.md`](./reference-artifact-inventory.md)。
...
| `美术/主面板/元素/MissionCard*.png`、`TaskCard*.png`、`Container*.png` | 版式与状态验收参考 | 含固定尺寸或烘焙内容，不覆盖动态任务和日志组件 |
| `美术/主面板/tag:button/Tag*`、`Tab*`、`StatesGrid*` | 控件状态验收参考 | 当前动态中文标签需自适应，直接套用固定尺寸图片会产生文字重叠 |
