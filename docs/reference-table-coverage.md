...
| `系统/配表_区域特殊剧情` | `AREA_SPECIAL_EVENTS`；`AREA_STORY_NARRATIVES`；`resolveAreaStoryCheck()`；`triggerAreaSpecialEvent()` | 运行中 | 24 条均具备场景、角色选择、D20 属性检定、成功/失败文本和差异化规则后果；后续仍可增加由模型在多个候选方案中主动选项的交互。 |
...
1. **P1：剧情专属选项**。关系状态、24 条区域剧情检定和三类模型调查路线已运行；下一步逐条接入参考表里的特有道具消耗与事件专属选择。
