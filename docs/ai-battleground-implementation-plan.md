...
| 内容规则 | `AREA_SPECIAL_EVENTS`、`storyOptionsFor()`、`GLOBAL_SPECIAL_EVENTS`、`relationshipEdges`、`ITEM_DEFINITIONS` | 24×3 区域剧情处置、3 条全局事件、剧情道具、关系戏剧、物品稀有度/价值、资源刷新和区域 buff 进入循环。 |
...
| 观众应援阵营 | `SupportFactionPanel.tsx`、`supportFaction.ts`、`submitSupportOrder()` | 本地保存应援身份、路线与声望；服务端权威保存 60 秒任务、投入、AI 回应、倒计时、执行和点数结算。猎手/后勤/情报分别强化追猎、搜集和谈判。 |
...
1. 为已上线的 24×3 事件专属处置继续增加人物人设驱动的差异后果；事件 ID、选项、参考表明确的道具前置和消费已经进入模型协议与规则层。
