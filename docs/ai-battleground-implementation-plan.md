...
每个新局写入 `seed` 与规则版本。服务器端随机数只能通过 `battleRandom()` 取得；`actionLog` 以独立 `nextActionId` 记录每次模型提交和规则回退，`replayCheckpoints` 每 30 秒写入轻量状态帧。状态帧包含观赛所需的角色位置/状态、区域、关系、资源、剧情和热度，不包含提示词、API 密钥或完整世界副本。
